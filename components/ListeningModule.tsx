  ...
  const fetchNewPhrase = async () => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: 'Give me one short and simple English sentence for a basic student. Maximum 8 words.',
      });
      setShadowingText(response.text?.trim().replace(/["']/g, '') || shadowingText);
    } catch (e) {
      console.error(e);
    } finally {
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-200">
        <div className="flex justify-between items-center mb-6">
          ...
          <button 
            ...
            className="text-xs font-bold text-indigo-600 uppercase tracking-widest hover:underline disabled:opacity-30"
          >
            {loadingNewPhrase ? 'Generando...' : 'Nueva Frase'}
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <a href="https://www.newsinslowenglish.com/" target="_blank" className="p-4 bg-white rounded-2xl border border-slate-200 flex items-center gap-4 hover:border-indigo-300 transition-all group">
        ...
        <a href="https://www.youtube.com/@EasyEnglishLearning" target="_blank" className="p-4 bg-white rounded-2xl border border-slate-200 flex items-center gap-4 hover:border-red-300 transition-all group">
